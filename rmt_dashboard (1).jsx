// NGA RMT Dashboard — Requirements Management Tool analytics surface.
// Consumes four ArcGIS REST services (dash_ProductLines, dash_Requirements,
// dash_Missions, dash_Counts) under /Dashboard/<office>/MapServer/<layerId>.
// Toggle USE_MOCK_DATA = false to point at live endpoints.
...
import {
  RefreshCw, Filter, X, Search, Download, ChevronRight, AlertTriangle,
  CheckCircle2, Clock, TrendingUp, TrendingDown, Minus, Info, Activity,
  Globe, Target, Layers, ArrowUpRight, FileText,
} from "lucide-react";
...
const REST_BASE = "https://dev.vcps.xc.nga.mil/fgnox-ags/rest/services/Dashboard";
const LAYER_IDS = { productlines: 0, requirements: 1, missions: 2, counts: 3 };

// ============================================================================
// DOMAIN VOCABULARIES — derived from the RMT schema
// ============================================================================
const FOUNDATION_DOMAINS = ["TOPO", "AERO", "MARITIME", "HUMAN_GEO", "TARGET"];
const AORS = ["AFRICOM", "CENTCOM", "EUCOM", "INDOPACOM", "NORTHCOM", "SOUTHCOM"];
const COUNTRIES_BY_AOR = {
  AFRICOM: ["NGA", "KEN", "ETH", "SDN", "COD", "ZAF", "MLI"],
  CENTCOM: ["IRQ", "IRN", "SYR", "AFG", "PAK", "YEM", "JOR"],
  EUCOM: ["UKR", "POL", "DEU", "FRA", "GBR", "TUR", "FIN"],
  INDOPACOM: ["CHN", "PRK", "JPN", "KOR", "PHL", "TWN", "VNM"],
  NORTHCOM: ["MEX", "CAN", "BHS", "CUB"],
  SOUTHCOM: ["VEN", "COL", "BRA", "ARG", "PER", "CHL"],
};
const PROD_STATUSES = ["Planned", "Initiated", "In Work", "Complete", "Cancelled", "On Hold"];
const PROD_STATUS_CODES = { Planned: 1, Initiated: 2, "In Work": 3, Complete: 4, Cancelled: 5, "On Hold": 6 };
const NSG_MEMBERS = ["NGA", "DIA", "NRO", "USAF", "USA", "USN", "USMC", "USSF", "DOS"];
const NEED_TYPES = ["Mission Critical", "Operational", "Strategic", "Tactical", "Routine"];
const INTENDED_USE = ["Targeting", "Mission Planning", "Navigation", "Analysis", "Briefing", "Training"];
const SATISFACTION = ["Fully Satisfied", "Partially Satisfied", "Unsatisfied", "Pending"];
const PROD_CLASSES = ["Standard", "Non-Standard", "Custom", "Derived"];
const CDAS = ["NGA-W", "NGA-E", "NGA-S", "AGC", "NIWC", "AFLCMC"];
const MISSION_NAMES = [
  "OPERATION ATLAS", "JOINT GUARDIAN", "STEEL SENTINEL", "PACIFIC SHIELD",
  "DESERT WATCH", "ARCTIC RECON", "BALTIC RESOLVE", "SOUTHERN CROSS",
  "PHOENIX RISE", "IRON HORIZON", "SILENT FALCON", "GLOBAL REACH",
];

// Office selector — the top-level filter that re-fetches all four services.
const OFFICES = [
  { code: "PO-AERO", label: "Aeronautical Production Office" },
  { code: "PO-TOPO", label: "Topographic Production Office" },
  { code: "PO-MAR", label: "Maritime Production Office" },
  { code: "PO-HG", label: "Human Geography Office" },
  { code: "PO-TGT", label: "Targeting Production Office" },
];

const FISCAL_YEARS = ["FY24", "FY25", "FY26"];

// ============================================================================
// DESIGN TOKENS — GeoQuery brand palette (executive: deep navy + forest + slate)
// ============================================================================
const C = {
  ink: "#d4d8cf",        // page background — muted slate-sage grey
  panel: "#eef0e8",      // card surface — soft warm grey with green tint
  panelHi: "#c9cec0",    // recessed track / inset surface
  rule: "#8a9974",       // sage hairline borders (saturated)
  ruleHi: "#3d5a1a",     // emphasized borders — deep forest
  text: "#0a1530",       // primary text — deep navy (richer than logo blue)
  textDim: "#2d3a5e",    // secondary
  textFaint: "#4d5a78",  // tertiary
  amber: "#16307a",      // PRIMARY ACCENT = deeper GeoQuery blue
  amberHi: "#1f3fa8",    // brand blue for hover/active
  lime: "#6b8a3a",       // KPI fill — deep forest sage
  limeDeep: "#3d5a1a",   // even deeper sage for chart use
  emerald: "#2f5418",    // complete / good — dark forest
  rust: "#8e2f15",       // unplanned / warning — deep oxide red
  rose: "#6b1030",       // high-priority unsatisfied — deep wine
  steel: "#1f3568",      // planning / neutral info — deep navy-blue
  violet: "#3f2a6b",     // strategic — deep aubergine
  cream: "#7a5a14",      // in-work — deep ochre
  invert: "#ffffff",     // contrast color for text/labels on filled backgrounds
};

// Status color is consistent everywhere it appears.
const STATUS_COLORS = {
  Planned: C.steel,
  Initiated: C.violet,
  "In Work": C.cream,
...
// ============================================================================
// MOCK DATA — internally consistent across the four services
// ============================================================================
...
function buildMockData(office) {
  // Use office code to perturb seed so different offices show different data.
  const officeSeed = office.charCodeAt(0) + office.charCodeAt(office.length - 1);
  const r = mulberry32(7 + officeSeed);

  // ----- Product lines (≥500) -----
  const productLines = [];
  for (let i = 0; i < 540; i++) {
    const aor = AORS[Math.floor(r() * AORS.length)];
    const countries = COUNTRIES_BY_AOR[aor];
    const status = pickW(
      PROD_STATUSES,
      [22, 14, 26, 28, 4, 6]
    );
    const fyCurrent = pickW(FISCAL_YEARS, [10, 70, 20]);
    const inProductionStat = ["Initiated", "In Work", "Complete"].includes(status);
    // Deviation flag per RMT rule: in production, current FY, not in plan.
    const inPlan = r() > 0.18;
    const deviation = inProductionStat && fyCurrent === "FY25" && !inPlan ? 1 : 0;
    productLines.push({
      objectid: i + 1,
      product: `PRD-${(10000 + i).toString()}`,
      product_id: `PID-${(20000 + i).toString()}`,
      foundation_domain: FOUNDATION_DOMAINS[Math.floor(r() * FOUNDATION_DOMAINS.length)],
      country: countries[Math.floor(r() * countries.length)],
      aor,
      prod_class_code: pick(PROD_CLASSES),
      cur_prod_stat: status,
      cur_prod_stat_code: PROD_STATUS_CODES[status],
      substat: r() > 0.6 ? `Sub-${Math.floor(r() * 5) + 1}` : null,
      fy_cur_prod_stat_code: fyCurrent,
      fy_subseq_plan: pick(FISCAL_YEARS),
      cda: pick(CDAS),
      avail_status: pickW(["Available", "Limited", "Restricted"], [70, 22, 8]),
      deviation,
      curr_year: 2025,
    });
  }
  ...
  const requirements = [];
  for (let i = 0; i < 240; i++) {
    const cust_pri = Math.floor(r() * 5) + 1; // 1-5
    const risk_factor = Math.floor(r() * 5) + 1; // 1-5
    const derived_pri = cust_pri <= 2 && risk_factor <= 2 ? 1 : 0;
    const sat = derived_pri
      ? pickW(SATISFACTION, [25, 30, 35, 10])
      : pickW(SATISFACTION, [55, 25, 12, 8]);
    requirements.push({
      objectid: i + 1,
      requirement_id: `REQ-${(30000 + i).toString()}`,
      nsg_member: pickW(NSG_MEMBERS, [40, 15, 8, 10, 8, 7, 4, 3, 5]),
      need_type: pick(NEED_TYPES),
      intended_use: pick(INTENDED_USE),
      strat_pri: Math.floor(r() * 5) + 1,
      cust_pri,
      risk_factor,
      nga_aa: r() > 0.4 ? "Approved" : "Pending",
      req_satisfaction: sat,
      derived_pri,
      cur_prod_stat: pick(PROD_STATUSES),
    });
  }

  // ----- Missions (≥400 links, many-to-many) -----
  const missions = [];
  let mid = 0;
  requirements.forEach((req) => {
    const linkCount = pickW([0, 1, 2, 3, 4, 5], [5, 25, 30, 22, 12, 6]);
    for (let k = 0; k < linkCount; k++) {
      missions.push({
        objectid: ++mid,
        requirement_id: req.requirement_id,
        mission_name: MISSION_NAMES[Math.floor(r() * MISSION_NAMES.length)],
        cust_pri: req.cust_pri,
        risk_factor: req.risk_factor,
        cur_prod_stat: req.cur_prod_stat,
  ...
  const counts = [];
  let cumulative = 0;
  const startMonth = new Date(2023, 9, 1); // Oct 2023 = start of FY24
  for (let m = 0; m < 24; m++) {
    const d = new Date(startMonth);
    d.setMonth(d.getMonth() + m);
    const inWork = Math.round(35 + r() * 45 + Math.sin(m / 3) * 12);
    const complete = Math.round(20 + r() * 35 + Math.cos(m / 4) * 10);
    cumulative += complete;
    counts.push({
      objectid: m + 1,
      month: d.toISOString().slice(0, 7),
      monthLabel: d.toLocaleDateString("en-US", { month: "short", year: "2-digit" }),
      count_in_work: inWork,
      count_complete: complete,
      cumulative_complete: cumulative,
    });
  }

  return { productLines, requirements, missions, counts };
}

// ============================================================================
// SERVICE LAYER — abstraction over the four REST endpoints.
// In live mode each function fetches GeoJSON and unwraps `properties`.
// ============================================================================
async function fetchService(office, layerId) {
  const url =
    `${REST_BASE}/${office}/MapServer/${layerId}/query` +
    `?where=1%3D1&outFields=*&returnGeometry=false&f=geojson`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Service ${layerId} failed: ${res.status}`);
  const json = await res.json();
  return (json.features || []).map((f) => f.properties);
}

async function loadAllServices(office) {
  if (USE_MOCK_DATA) {
    await new Promise((r) => setTimeout(r, 280));
    return buildMockData(office);
  }
  const [productLines, requirements, missions, counts] = await Promise.all([
    fetchService(office, LAYER_IDS.productlines),
    fetchService(office, LAYER_IDS.requirements),
    fetchService(office, LAYER_IDS.missions),
    fetchService(office, LAYER_IDS.counts),
  ]);
  return { productLines, requirements, missions, counts };
}

// ============================================================================
// PURE METRIC FUNCTIONS — separated from rendering, individually testable.
// ============================================================================
...
function applyFilters(data, filters) {
  const matches = (val, sel) => sel.length === 0 || sel.includes(val);
  const productLines = data.productLines.filter(
    (p) =>
      matches(p.foundation_domain, filters.domain) &&
      matches(p.aor, filters.aor) &&
      matches(p.country, filters.country) &&
      matches(p.fy_cur_prod_stat_code, filters.fy) &&
      matches(p.cur_prod_stat, filters.status)
  );
  const requirements = data.requirements.filter(
    (r) =>
      matches(r.nsg_member, filters.nsg) &&
      matches(r.cur_prod_stat, filters.status) &&
      (filters.priority.length === 0 ||
        (filters.priority.includes("High") && r.derived_pri === 1) ||
        (filters.priority.includes("Standard") && r.derived_pri === 0))
  );
  const reqIds = new Set(requirements.map((r) => r.requirement_id));
  const missions = data.missions.filter((m) => reqIds.has(m.requirement_id));
  return { productLines, requirements, missions, counts: data.counts };
}

function computeKPIs(filtered) {
  const { productLines, requirements } = filtered;
  const total = productLines.length;
  const completeFY = productLines.filter(
    (p) => p.cur_prod_stat === "Complete" && p.fy_cur_prod_stat_code === "FY25"
  ).length;
  const inWork = productLines.filter((p) => p.cur_prod_stat === "In Work").length;
  const unplanned = productLines.filter((p) => p.deviation === 1).length;
  const totalReqs = requirements.length;
  const highPriUnsat = requirements.filter(
    (r) => r.derived_pri === 1 && r.req_satisfaction !== "Fully Satisfied"
  ).length;
  return {
    total,
    pctComplete: pct(completeFY, total),
    inWork,
    unplanned,
    pctUnplanned: pct(unplanned, total),
    totalReqs,
    highPriUnsat,
    pctHighPriUnsat: pct(highPriUnsat, totalReqs),
  };
}

function generateInsights(filtered, kpis) {
  const insights = [];
  const { productLines, requirements } = filtered;

  // Insight 1: AOR concentration of unplanned production
  const unplanned = productLines.filter((p) => p.deviation === 1);
  if (unplanned.length > 0) {
    const byAor = groupCount(unplanned, "aor");
    const top = Object.entries(byAor).sort((a, b) => b[1] - a[1])[0];
    const aorTotal = productLines.filter((p) => p.aor === top[0]).length;
    insights.push({
      tone: "warn",
      text: `${top[0]} accounts for ${((top[1] / unplanned.length) * 100).toFixed(0)}% of unplanned production (${top[1]} of ${unplanned.length}) while representing ${((aorTotal / productLines.length) * 100).toFixed(0)}% of total volume.`,
    });
  }

  // Insight 2: Domain with highest unsatisfied high-priority rate
  if (requirements.length > 0) {
    const domainStats = {};
    requirements.forEach((r) => {
      // Approximate domain via correlated product line — use need_type as proxy bucket
      const k = r.need_type;
      if (!domainStats[k]) domainStats[k] = { hp: 0, hpUnsat: 0 };
      if (r.derived_pri === 1) {
        domainStats[k].hp++;
        if (r.req_satisfaction !== "Fully Satisfied") domainStats[k].hpUnsat++;
      }
    });
    const ranked = Object.entries(domainStats)
      .filter(([, v]) => v.hp >= 3)
      .map(([k, v]) => [k, (v.hpUnsat / v.hp) * 100, v.hp])
      .sort((a, b) => b[1] - a[1]);
    if (ranked.length > 0) {
      const [need, rate, n] = ranked[0];
      insights.push({
        tone: "alert",
        text: `${rate.toFixed(0)}% of high-priority "${need}" requirements are unsatisfied (${Math.round((rate / 100) * n)} of ${n}) — the highest of any need-type bucket.`,
      });
    }
  }

  // Insight 3: Production velocity month-over-month
  const counts = filtered.counts;
  if (counts.length >= 3) {
    const last = counts[counts.length - 1];
    const prev = counts[counts.length - 2];
    const delta = pct(last.count_complete - prev.count_complete, prev.count_complete);
    insights.push({
      tone: delta < -10 ? "warn" : delta > 10 ? "good" : "neutral",
      text: `Completion velocity ${delta >= 0 ? "rose" : "fell"} ${Math.abs(delta).toFixed(0)}% in ${last.monthLabel} (${last.count_complete} vs ${prev.count_complete} the prior month).`,
    });
  }

  // Insight 4: CDA workload balance
  const cdaCount = groupCount(productLines, "cda");
  const cdaSorted = Object.entries(cdaCount).sort((a, b) => b[1] - a[1]);
  if (cdaSorted.length >= 2) {
    const ratio = cdaSorted[0][1] / cdaSorted[cdaSorted.length - 1][1];
    if (ratio > 2) {
      insights.push({
        tone: "neutral",
        text: `CDA workload is uneven — ${cdaSorted[0][0]} is carrying ${ratio.toFixed(1)}× the volume of the lowest-loaded CDA (${cdaSorted[cdaSorted.length - 1][0]}).`,
      });
    }
  }

  // Insight 5: KPI summary
  if (kpis.total > 0) {
    insights.push({
      tone: "neutral",
      text: `${kpis.pctComplete.toFixed(0)}% of FY25 product lines are complete; ${kpis.pctUnplanned.toFixed(0)}% are unplanned intrusions outside the production plan.`,
    });
  }

  // Insight 6: Mission load
  const missionCount = filtered.missions.length;
  const linkedReqs = new Set(filtered.missions.map((m) => m.requirement_id)).size;
  if (linkedReqs > 0) {
    const avg = (missionCount / linkedReqs).toFixed(1);
    insights.push({
      tone: "good",
      text: `${linkedReqs} requirements are linked to ${missionCount} mission instances (avg ${avg} missions per requirement).`,
    });
  }
...
const Tag = ({ children, tone = "default", onRemove }) => {
  const tones = {
    default: { bg: "transparent", border: C.rule, text: C.textDim },
    active: { bg: "rgba(212,160,76,0.08)", border: C.amber, text: C.amber },
    warn: { bg: "rgba(184,84,61,0.08)", border: C.rust, text: C.rust },
  };
  const t = tones[tone];
  return (
    <span
      className="inline-flex items-center gap-1.5 px-2 py-0.5 text-[12px] tracking-[0.12em] uppercase font-mono font-semibold"
      style={{ background: t.bg, border: `1px solid ${t.border}`, color: t.text }}
    >
      {children}
      {onRemove && (
        <button onClick={onRemove} className="hover:opacity-70" aria-label="Remove filter">
          <X size={10} />
        </button>
      )}
    </span>
...
const Card = ({ children, className = "", title, subtitle, hint, action }) => (
  <div
      ...
      <>
        <div className="flex items-start justify-between px-5 pt-4 pb-3">
          <div>
            {title && (
              <div
                className="text-[12px] tracking-[0.16em] uppercase font-mono font-semibold flex items-center gap-2"
                style={{ color: C.text }}
              >
                {title}
                {hint && (
            ...
            {subtitle && (
              <div className="text-[12px] mt-1 italic" style={{ color: C.textDim }}>
                {subtitle}
              </div>
            )}
          </div>
...
const KPITile = ({ label, value, suffix, sub, spark, sparkColor, tone = "default", icon: Icon }) => {
  const accent = tone === "warn" ? C.rust : tone === "alert" ? C.rose : tone === "good" ? C.emerald : C.amber;
  // Lime fill for default/good tones, white for warning tones (so red/rose pop)
  const tileBg = tone === "warn" || tone === "alert" ? "#ffffff" : C.lime;
  const tileText = tone === "warn" || tone === "alert" ? C.text : C.invert;
  const tileSub = tone === "warn" || tone === "alert" ? C.textDim : "rgba(255,255,255,0.78)";
  return (
    <div
      className="relative p-5 flex flex-col justify-between min-h-[140px]"
      style={{ background: tileBg, border: `1px solid ${tone === "warn" || tone === "alert" ? accent : C.ruleHi}`, borderRadius: "8px" }}
    >
      <div className="flex items-start justify-between">
        <div
          className="text-[12px] tracking-[0.16em] uppercase font-mono font-semibold"
          style={{ color: tileText }}
        >
          {label}
        </div>
        {Icon && <Icon size={16} style={{ color: tone === "warn" || tone === "alert" ? accent : C.invert }} strokeWidth={2.2} />}
      </div>
      <div className="flex items-end justify-between mt-3">
        <div>
          <div
            className="font-serif tabular-nums font-semibold"
            style={{ color: tileText, fontSize: "42px", lineHeight: 1, letterSpacing: "-0.02em" }}
          >
            {value}
            {suffix && (
              <span
                className="text-[20px] ml-1 font-sans font-medium"
                style={{ color: tileText, opacity: 0.85 }}
              >
                {suffix}
              </span>
            )}
          ...
          {sub && (
            <div className="text-[11px] mt-2 font-mono uppercase tracking-wider font-medium" style={{ color: tileSub }}>
              {sub}
            </div>
          )}
        </div>
        {spark && (
          <div className="w-24 opacity-95">
            <Spark data={spark} color={tone === "warn" || tone === "alert" ? (sparkColor || accent) : C.invert} />
          </div>
        )}
      </div>
    </div>
...
// Custom tooltip for charts — matches the dashboard aesthetic
  ...
  return (
    <div
      style={{
        ...
        boxShadow: "0 4px 12px rgba(13,31,71,0.12)",
        padding: "8px 10px",
        fontFamily: "ui-monospace, monospace",
      }}
    >
      {label && (
        <div className="text-[12px] tracking-wider uppercase mb-1.5 font-semibold" style={{ color: C.text }}>
          {label}
        </div>
      )}
      {payload.map((p, i) => (
        <div key={i} className="text-[13px] flex items-center gap-2" style={{ color: C.text }}>
          <span
            style={{ display: "inline-block", width: 8, height: 8, background: p.color || p.fill }}
          />
          <span style={{ color: C.textDim }}>{p.name}:</span>
          <span className="tabular-nums font-semibold">{formatter ? formatter(p.value) : p.value}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="mb-4">
      <button
        ...
        className="w-full flex items-center justify-between text-[12px] tracking-[0.16em] uppercase font-mono font-semibold py-2 hover:opacity-100"
        style={{ color: C.text }}
      >
        ...
        {selected.length > 0 && (
          <span className="font-bold" style={{ color: C.amber }}>{selected.length}</span>
        )}
      </button>
      {expanded && (
          ...
          {options.map((opt) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-2.5 px-2 py-1.5 text-[12px] text-left hover:bg-black/5"
                style={{
              ...
              >
                <span
                  style={{
                    width: 11,
                    height: 11,
                    border: `1.5px solid ${isSel ? C.amber : C.rule}`,
                    background: isSel ? C.amber : "transparent",
                    flexShrink: 0,
                  }}
                />
                <span className="truncate font-mono tracking-wide">{opt}</span>
              </button>
            );
          })}
        </div>
...
// PRODUCTION TAB
// ============================================================================
const ProductionTab = ({ filtered }) => {
  const { productLines, counts } = filtered;

  const statusBreakdown = useMemo(() => {
    const g = groupCount(productLines, "cur_prod_stat");
    return PROD_STATUSES.map((s) => ({ name: s, value: g[s] || 0 })).filter((d) => d.value > 0);
  }, [productLines]);

  const deviationByDomain = useMemo(() => {
    const g = {};
    productLines.forEach((p) => {
      if (!g[p.foundation_domain]) g[p.foundation_domain] = { domain: p.foundation_domain, deviation: 0, planned: 0 };
      if (p.deviation === 1) g[p.foundation_domain].deviation++;
      else g[p.foundation_domain].planned++;
    });
    return Object.values(g);
  }, [productLines]);

  const aorCountryHeat = useMemo(() => {
    const data = AORS.map((aor) => {
      const aorRows = productLines.filter((p) => p.aor === aor);
      return {
        aor,
        total: aorRows.length,
        complete: aorRows.filter((p) => p.cur_prod_stat === "Complete").length,
        inWork: aorRows.filter((p) => p.cur_prod_stat === "In Work").length,
        deviation: aorRows.filter((p) => p.deviation === 1).length,
      };
    }).filter((d) => d.total > 0);
    return data;
  }, [productLines]);

  const cdaBreakdown = useMemo(() => {
    const g = {};
    productLines.forEach((p) => {
      if (!g[p.cda]) g[p.cda] = { cda: p.cda, complete: 0, inWork: 0, planned: 0, other: 0 };
      const s = p.cur_prod_stat;
      if (s === "Complete") g[p.cda].complete++;
      else if (s === "In Work") g[p.cda].inWork++;
      else if (s === "Planned") g[p.cda].planned++;
      else g[p.cda].other++;
    });
    return Object.values(g);
  }, [productLines]);
  ...
  return (
    <div className="grid grid-cols-12 gap-px" style={{ background: C.rule }}>
      <Card
        className="col-span-12"
        title="Geographic Distribution"
        subtitle="Product line volume by country · bubble size = total volume · inner red = unplanned share"
        hint="Aggregated from dash_ProductLines.country (GENC alpha-3). Hover any country with data for breakdown."
      >
        <GeoMap productLines={productLines} />
      </Card>

      <Card
        className="col-span-12 lg:col-span-8"
        title="Monthly Production Burndown"
        subtitle="In-work vs completed products by month, last 24 months"
        hint="Sourced from dash_Counts. count_in_work = cur_prod_stat_code 3; count_complete = code 4."
      >
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={counts} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
            <CartesianGrid stroke={C.rule} strokeDasharray="0" vertical={false} />
            <XAxis
              dataKey="monthLabel"
              tick={{ fill: C.textFaint, fontSize: 12, fontFamily: "ui-monospace", fontWeight: 500 }}
            ...
            <YAxis
              ...
              tickLine={false}
            />
            <Tooltip content={<ChartTip />} cursor={{ stroke: C.ruleHi }} />
            <Legend
              wrapperStyle={{ fontSize: 12, fontFamily: "ui-monospace", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.1em" }}
            />
            <Line
              type="monotone"
              dataKey="count_in_work"
              name="In Work"
              stroke={C.cream}
              strokeWidth={1.5}
              dot={false}
      ...
      <Card
        ...
        subtitle="Distribution across cur_prod_stat"
      >
        <ResponsiveContainer width="100%" height={240}>
          <PieChart>
            <Pie
        ...
        <div className="mt-2 grid grid-cols-2 gap-1">
          {statusBreakdown.map((d) => (
            <div key={d.name} className="flex items-center gap-1.5 text-[12px] font-mono" style={{ color: C.textDim }}>
              <span style={{ width: 8, height: 8, background: STATUS_COLORS[d.name] }} />
              <span className="truncate">{d.name}</span>
              <span className="tabular-nums ml-auto" style={{ color: C.text }}>{d.value}</span>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card
        ...
        subtitle="Running total across the period"
        hint="cumulative_complete from dash_Counts."
      >
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={counts} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
            <defs>
      ...
      <Card
        ...
        title="Unplanned Production"
        subtitle="Deviation flag by foundation domain"
        hint="deviation = 1 when product is in production this FY but absent from plan."
      >
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={deviationByDomain} margin={{ top: 8, right: 12, left: -8, bottom: 0 }} barCategoryGap={"30%"}>
            <CartesianGrid stroke={C.rule} vertical={false} />
            <XAxis
              dataKey="domain"
              tick={{ fill: C.textFaint, fontSize: 12, fontFamily: "ui-monospace", fontWeight: 500 }}
            ...
            <Legend
              wrapperStyle={{ fontSize: 12, fontFamily: "ui-monospace", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.1em" }}
            />
            <Bar dataKey="planned" name="Planned" stackId="a" fill={C.steel} />
            <Bar dataKey="deviation" name="Unplanned" stackId="a" fill={C.rust} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
      ...
      <Card
        className="col-span-12 lg:col-span-7"
        title="AOR × Status Heatmap"
        subtitle="Production volume and status distribution by area of responsibility"
      >
        <div className="space-y-1.5">
          {aorCountryHeat.map((row) => {
            const max = Math.max(...aorCountryHeat.map((r) => r.total));
            const totalPct = (row.total / max) * 100;
            const completePct = (row.complete / row.total) * 100;
            const inWorkPct = (row.inWork / row.total) * 100;
            const devPct = (row.deviation / row.total) * 100;
            return (
              <div key={row.aor} className="grid grid-cols-12 gap-2 items-center">
                <div className="col-span-2 text-[12px] font-mono uppercase tracking-wider font-semibold" style={{ color: C.textDim }}>
                  {row.aor}
                </div>
                <div className="col-span-8 relative h-5" style={{ background: C.panelHi, border: `1px solid ${C.rule}` }}>
                  <div
                    className="absolute h-full"
                    style={{ width: `${totalPct}%`, background: C.panelHi, border: `1px solid ${C.rule}` }}
                  />
                  <div className="absolute inset-0 flex">
                    <div style={{ width: `${(row.complete / max) * 100}%`, background: C.emerald, opacity: 0.85 }} />
                    <div style={{ width: `${(row.inWork / max) * 100}%`, background: C.cream, opacity: 0.85 }} />
                    <div style={{ width: `${(row.deviation / max) * 100}%`, background: C.rust, opacity: 0.85 }} />
                  </div>
                </div>
                <div className="col-span-2 text-right text-[13px] font-mono tabular-nums" style={{ color: C.text }}>
                  {row.total}
                  <span style={{ color: C.textFaint }} className="ml-1.5 text-[9px]">
                    ({completePct.toFixed(0)}%C)
                  </span>
                </div>
              </div>
            );
        ...
        <div className="flex gap-3 mt-3 pt-3" style={{ borderTop: `1px solid ${C.rule}` }}>
          {[
            ["Complete", C.emerald],
            ["In Work", C.cream],
            ["Unplanned", C.rust],
          ].map(([l, c]) => (
            <div key={l} className="flex items-center gap-1.5 text-[12px] font-mono uppercase tracking-wider font-semibold" style={{ color: C.textDim }}>
      ...
      <Card
        className="col-span-12 lg:col-span-5"
        title="CDA Production Load"
        subtitle="Volume by Content Development Activity"
      >
        <ResponsiveContainer width="100%" height={210}>
          <BarChart data={cdaBreakdown} layout="vertical" margin={{ top: 4, right: 12, left: 28, bottom: 0 }}>
            ...
            <YAxis
              dataKey="cda"
              type="category"
              tick={{ fill: C.textDim, fontSize: 12, fontFamily: "ui-monospace", fontWeight: 500 }}
              axisLine={false}
              tickLine={false}
            />
            <Tooltip content={<ChartTip />} cursor={{ fill: "rgba(255,255,255,0.03)" }} />
            <Bar dataKey="complete" name="Complete" stackId="a" fill={STATUS_COLORS.Complete} />
            <Bar dataKey="inWork" name="In Work" stackId="a" fill={STATUS_COLORS["In Work"]} />
            <Bar dataKey="planned" name="Planned" stackId="a" fill={STATUS_COLORS.Planned} />
            <Bar dataKey="other" name="Other" stackId="a" fill={C.textFaint} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
    </div>
...
// REQUIREMENTS TAB
// ============================================================================
const RequirementsTab = ({ filtered }) => {
  const { requirements } = filtered;

  // Priority matrix: 5x5 cust_pri × risk_factor, cell shows count + satisfaction rate
  const priorityMatrix = useMemo(() => {
    const grid = [];
    for (let cp = 1; cp <= 5; cp++) {
      for (let rf = 1; rf <= 5; rf++) {
        const cell = requirements.filter((r) => r.cust_pri === cp && r.risk_factor === rf);
        const sat = cell.filter((r) => r.req_satisfaction === "Fully Satisfied").length;
        grid.push({
          cust_pri: cp,
          risk_factor: rf,
          count: cell.length,
          satRate: cell.length > 0 ? (sat / cell.length) * 100 : null,
        });
      }
    }
    return grid;
  ...
  const maxCellCount = Math.max(...priorityMatrix.map((c) => c.count), 1);

  // NSG stacked by need_type
  const nsgByNeed = useMemo(() => {
    const g = {};
    NSG_MEMBERS.forEach((m) => {
      g[m] = { nsg_member: m };
      NEED_TYPES.forEach((n) => (g[m][n] = 0));
    });
    requirements.forEach((r) => {
      if (g[r.nsg_member]) g[r.nsg_member][r.need_type] = (g[r.nsg_member][r.need_type] || 0) + 1;
    });
    return Object.values(g).filter((row) => NEED_TYPES.some((n) => row[n] > 0));
  }, [requirements]);

  // Satisfaction funnel
  const satFunnel = useMemo(() => {
    const g = groupCount(requirements, "req_satisfaction");
    return SATISFACTION.map((s) => ({ name: s, value: g[s] || 0 }));
  }, [requirements]);

  // Top-10 unsatisfied high-priority
  const topUnsat = useMemo(() => {
    return requirements
      .filter((r) => r.derived_pri === 1 && r.req_satisfaction !== "Fully Satisfied")
      .sort((a, b) => a.cust_pri - b.cust_pri || a.risk_factor - b.risk_factor)
      .slice(0, 10);
  }, [requirements]);

  // Intended use treemap
  const useTreemap = useMemo(() => {
    const g = groupCount(requirements, "intended_use");
    return Object.entries(g).map(([name, value], i) => ({
      name,
      size: value,
      fill: [C.amber, C.steel, C.violet, C.emerald, C.cream, C.rose][i % 6],
    }));
  }, [requirements]);

  const NEED_COLORS = [C.amber, C.steel, C.violet, C.emerald, C.cream];

  return (
    <div className="grid grid-cols-12 gap-px" style={{ background: C.rule }}>
      <Card
        className="col-span-12 lg:col-span-7"
        title="Customer Priority × Risk Factor"
        subtitle="Cell size = requirement count · Color = satisfaction rate"
        hint="derived_pri flag triggers when both cust_pri ≤ 2 AND risk_factor ≤ 2."
      >
        <div className="flex">
          <div className="flex flex-col justify-center pr-3">
            <div
              className="text-[9px] uppercase tracking-[0.2em] font-mono whitespace-nowrap"
              style={{ color: C.textFaint, transform: "rotate(-90deg)", transformOrigin: "center" }}
            >
              Cust Pri →
            </div>
          </div>
          <div className="flex-1">
            <div className="grid grid-cols-5 gap-1">
              {[1, 2, 3, 4, 5].map((cp) =>
                [1, 2, 3, 4, 5].map((rf) => {
                  const cell = priorityMatrix.find((c) => c.cust_pri === cp && c.risk_factor === rf);
                  const sizePct = (cell.count / maxCellCount) * 100;
                  const satColor =
                    cell.satRate === null
                      ? C.panelHi
                      : cell.satRate >= 70
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: `1px solid ${isHP ? C.amber : C.rule}`,
                      }}
                      title={`Cust ${cp} / Risk ${rf}: ${cell.count} reqs, ${cell.satRate?.toFixed(0) ?? "—"}% satisfied`}
                    >
                      {cell.count > 0 && (
                        <div
                          className="absolute"
                          style={{
                            width: `${Math.max(sizePct, 18)}%`,
                            height: `${Math.max(sizePct, 18)}%`,
                            background: satColor,
                            opacity: 0.9,
                          }}
                        />
                      ...
                      <span
                        ...
                        style={{ color: cell.count > 0 ? C.invert : C.textFaint, fontWeight: 700 }}
                      >
                        {cell.count}
                      </span>
                    </div>
            ...
            <div className="text-center text-[9px] uppercase tracking-[0.2em] font-mono mt-2" style={{ color: C.textFaint }}>
              Risk Factor →
            </div>
          </div>
        </div>
      ...
      <Card
        className="col-span-12 lg:col-span-5"
        title="Satisfaction Funnel"
        subtitle="Requirement satisfaction state distribution"
      >
        <div className="space-y-1.5 mt-1">
          {satFunnel.map((s) => {
            const total = satFunnel.reduce((a, b) => a + b.value, 0);
            const p = total > 0 ? (s.value / total) * 100 : 0;
            const color =
              s.name === "Fully Satisfied"
                ? C.emerald
                : s.name === "Partially Satisfied"
                ? C.cream
                : s.name === "Pending"
                ? C.steel
                : C.rose;
            return (
              <div key={s.name}>
                <div className="flex justify-between text-[12px] font-mono uppercase tracking-wider font-semibold mb-1" style={{ color: C.textDim }}>
                  <span>{s.name}</span>
                  <span className="tabular-nums" style={{ color: C.text }}>
                    {s.value} <span style={{ color: C.textFaint }}>· {p.toFixed(0)}%</span>
                  </span>
                </div>
                <div className="h-3 relative" style={{ background: C.panelHi, border: `1px solid ${C.rule}` }}>
                  <div className="absolute inset-y-0 left-0" style={{ width: `${p}%`, background: color }} />
                </div>
              </div>
            );
          })}
      ...
      <Card
        className="col-span-12 lg:col-span-7"
        title="NSG Member × Need Type"
        subtitle="Requirements stacked by need-type within each NSG owner"
      >
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={nsgByNeed} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
            <CartesianGrid stroke={C.rule} vertical={false} />
            <XAxis
              dataKey="nsg_member"
              tick={{ fill: C.textFaint, fontSize: 12, fontFamily: "ui-monospace", fontWeight: 500 }}
            ...
            />
            {NEED_TYPES.map((n, i) => (
              <Bar key={n} dataKey={n} stackId="a" fill={NEED_COLORS[i]} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </Card>
      ...
      <Card
        className="col-span-12 lg:col-span-5"
        title="Intended Use"
        subtitle="Treemap of requirement intended-use values"
      >
        <ResponsiveContainer width="100%" height={240}>
          <Treemap
            data={useTreemap}
      ...
      <Card
        className="col-span-12"
        title="Top High-Priority Unsatisfied Requirements"
        subtitle="derived_pri = 1 AND req_satisfaction ≠ Fully Satisfied"
        action={
          <button
            className="flex items-center gap-1.5 text-[12px] font-mono uppercase tracking-wider font-semibold hover:opacity-100"
            style={{ color: C.amber }}
            onClick={() => exportCSV(topUnsat, "high_priority_unsatisfied")}
          >
      ...
      >
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] font-mono">
            <thead>
              <tr style={{ color: C.text }} className="text-left uppercase tracking-wider">
                <th className="py-1.5 pr-3 font-semibold text-[11px]">Req ID</th>
                <th className="py-1.5 px-3 font-semibold text-[11px]">NSG</th>
                <th className="py-1.5 px-3 font-semibold text-[11px]">Need Type</th>
                <th className="py-1.5 px-3 font-semibold text-[11px]">Intended Use</th>
                <th className="py-1.5 px-3 font-semibold text-[11px]">Cust Pri</th>
                <th className="py-1.5 px-3 font-semibold text-[11px]">Risk</th>
                <th className="py-1.5 px-3 font-semibold text-[11px]">Status</th>
                <th className="py-1.5 px-3 font-semibold text-[11px]">Satisfaction</th>
              </tr>
            </thead>
            <tbody>
              {topUnsat.map((r, i) => (
                <tr key={r.requirement_id} style={{ borderTop: `1px solid ${C.rule}`, color: C.text }}>
                  <td className="py-2 pr-3" style={{ color: C.amber }}>{r.requirement_id}</td>
                  <td className="py-2 px-3">{r.nsg_member}</td>
                  <td className="py-2 px-3" style={{ color: C.textDim }}>{r.need_type}</td>
                  <td className="py-2 px-3" style={{ color: C.textDim }}>{r.intended_use}</td>
                  <td className="py-2 px-3 tabular-nums">{r.cust_pri}</td>
                  <td className="py-2 px-3 tabular-nums">{r.risk_factor}</td>
                  <td className="py-2 px-3">
                    <span style={{ color: STATUS_COLORS[r.cur_prod_stat] || C.textDim }}>{r.cur_prod_stat}</span>
                  </td>
                  <td className="py-2 px-3">
                    <Tag tone="warn">{r.req_satisfaction}</Tag>
                  </td>
                </tr>
              ))}
              {topUnsat.length === 0 && (
                <tr>
                  <td colSpan={8} className="py-6 text-center" style={{ color: C.textFaint }}>
                    No high-priority unsatisfied requirements match current filters.
                  </td>
                </tr>
              )}
            </tbody>
...
// Custom treemap node — minimal labels
  ...
  return (
    <g>
      <rect
        ...
        width={width}
        height={height}
        style={{ fill, stroke: C.panel, strokeWidth: 2 }}
      />
      {width > 60 && height > 30 && (
        <>
          <text
...
// MISSIONS TAB
// ============================================================================
const MissionsTab = ({ filtered }) => {
  const { missions, requirements } = filtered;

  // Histogram: missions per requirement
  const missionsPerReq = useMemo(() => {
    const counts = {};
    requirements.forEach((r) => (counts[r.requirement_id] = 0));
    missions.forEach((m) => {
      counts[m.requirement_id] = (counts[m.requirement_id] || 0) + 1;
    });
    const buckets = {};
    Object.values(counts).forEach((c) => {
      const k = c >= 5 ? "5+" : String(c);
      buckets[k] = (buckets[k] || 0) + 1;
    });
    return ["0", "1", "2", "3", "4", "5+"].map((k) => ({ bucket: k, count: buckets[k] || 0 }));
  }, [missions, requirements]);

  // Mission by priority stacked bar
  const missionByPriority = useMemo(() => {
    const g = {};
    missions.forEach((m) => {
      const k = m.mission_name;
      if (!g[k]) g[k] = { mission: k, P1: 0, P2: 0, P3: 0, P4: 0, P5: 0 };
      g[k][`P${m.cust_pri}`] = (g[k][`P${m.cust_pri}`] || 0) + 1;
    });
    return Object.values(g)
      .map((row) => ({ ...row, total: row.P1 + row.P2 + row.P3 + row.P4 + row.P5 }))
      .sort((a, b) => b.total - a.total)
      .slice(0, 10);
  }, [missions]);

  // Top missions by linked requirement count
  const topMissions = useMemo(() => {
    const g = {};
    missions.forEach((m) => {
      if (!g[m.mission_name]) g[m.mission_name] = new Set();
      g[m.mission_name].add(m.requirement_id);
    });
    return Object.entries(g)
      .map(([name, reqs]) => ({ mission: name, reqs: reqs.size }))
      .sort((a, b) => b.reqs - a.reqs)
      .slice(0, 10);
  }, [missions]);

  // Mission-to-status matrix
  const missionStatus = useMemo(() => {
    const topMissionNames = topMissions.slice(0, 8).map((m) => m.mission);
    return topMissionNames.map((name) => {
      const cell = { mission: name };
      PROD_STATUSES.forEach((s) => (cell[s] = 0));
      missions.filter((m) => m.mission_name === name).forEach((m) => {
        cell[m.cur_prod_stat] = (cell[m.cur_prod_stat] || 0) + 1;
      });
      return cell;
    });
  ...
  return (
    <div className="grid grid-cols-12 gap-px" style={{ background: C.rule }}>
      <Card
        className="col-span-12 lg:col-span-5"
        title="Missions per Requirement"
        subtitle="Distribution of mission-link counts across requirements"
        hint="Each row in dash_Missions is one requirement↔mission link."
      >
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={missionsPerReq} margin={{ top: 8, right: 12, left: -8, bottom: 0 }} barCategoryGap={"35%"}>
            <CartesianGrid stroke={C.rule} vertical={false} />
            <XAxis
              dataKey="bucket"
              tick={{ fill: C.textFaint, fontSize: 12, fontFamily: "ui-monospace", fontWeight: 500 }}
            ...
            />
            <Tooltip content={<ChartTip />} cursor={{ fill: "rgba(255,255,255,0.03)" }} />
            <Bar dataKey="count" name="Requirements" fill={C.amber} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
      ...
      <Card
        className="col-span-12 lg:col-span-7"
        title="Mission Workload by Customer Priority"
        subtitle="Top 10 missions, links stacked by cust_pri"
      >
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={missionByPriority} margin={{ top: 8, right: 12, left: -8, bottom: 30 }}>
            <CartesianGrid stroke={C.rule} vertical={false} />
            <XAxis
              dataKey="mission"
              tick={{ fill: C.textFaint, fontSize: 11, fontFamily: "ui-monospace", fontWeight: 500 }}
              axisLine={{ stroke: C.rule }}
              tickLine={false}
              angle={-30}
              textAnchor="end"
            ...
            <Legend
              wrapperStyle={{ fontSize: 12, fontFamily: "ui-monospace", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.1em" }}
            />
            <Bar dataKey="P1" name="P1" stackId="a" fill={C.rose} />
            <Bar dataKey="P2" name="P2" stackId="a" fill={C.amber} />
            <Bar dataKey="P3" name="P3" stackId="a" fill={C.cream} />
            <Bar dataKey="P4" name="P4" stackId="a" fill={C.steel} />
            <Bar dataKey="P5" name="P5" stackId="a" fill={C.textFaint} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
      ...
      <Card
        className="col-span-12 lg:col-span-5"
        title="Top Missions by Requirement Count"
        subtitle="Distinct requirements linked to each mission"
      >
        <div className="space-y-1.5">
          {topMissions.map((m, i) => {
            const max = Math.max(...topMissions.map((x) => x.reqs));
            const p = (m.reqs / max) * 100;
            return (
              <div key={m.mission} className="grid grid-cols-12 gap-2 items-center">
                <div className="col-span-1 text-[12px] font-mono" style={{ color: C.textFaint }}>
                  {String(i + 1).padStart(2, "0")}
                </div>
                <div className="col-span-6 text-[11px] font-mono uppercase tracking-wider truncate" style={{ color: C.text }}>
                  {m.mission}
                </div>
                <div className="col-span-4 h-2 relative" style={{ background: C.panelHi, border: `1px solid ${C.rule}` }}>
                  <div className="absolute inset-y-0 left-0" style={{ width: `${p}%`, background: C.amber }} />
                </div>
                <div className="col-span-1 text-right text-[13px] font-mono tabular-nums" style={{ color: C.text }}>
                  {m.reqs}
                </div>
              </div>
            );
      ...
      <Card
        className="col-span-12 lg:col-span-7"
        title="Mission × Production Status"
        subtitle="Production status mix for top 8 missions"
      >
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] font-mono">
            <thead>
              <tr style={{ color: C.text }} className="text-left uppercase tracking-wider">
                <th className="py-2 pr-3 font-semibold text-[11px]">Mission</th>
                {PROD_STATUSES.map((s) => (
                  <th key={s} className="py-2 px-2 font-semibold text-[11px] text-right" style={{ color: STATUS_COLORS[s] }}>
                    {s.split(" ")[0]}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {missionStatus.map((row) => {
                const total = PROD_STATUSES.reduce((a, s) => a + (row[s] || 0), 0);
                return (
                  <tr key={row.mission} style={{ borderTop: `1px solid ${C.rule}`, color: C.text }}>
                    <td className="py-2 pr-3 truncate uppercase tracking-wider" style={{ maxWidth: 140 }}>
                      {row.mission}
                    </td>
                    {PROD_STATUSES.map((s) => {
                      const v = row[s] || 0;
                      ...
                      return (
                        <td key={s} className="py-2 px-2 text-right tabular-nums" style={{
                          background: intensity > 0 ? `${STATUS_COLORS[s]}${Math.round(intensity * 200).toString(16).padStart(2, "0")}` : "transparent",
                          color: C.text,
                        }}>
                          {v || ""}
                        </td>
...
// Reverse lookup: country GENC3 → AOR
const COUNTRY_TO_AOR = (() => {
  const m = {};
  Object.entries(COUNTRIES_BY_AOR).forEach(([aor, countries]) => {
    countries.forEach((c) => (m[c] = aor));
  });
  return m;
})();
...
// ============================================================================
// Pattern-matched parser. Returns { filters, sql, summary, matched }.
// In production, swap parseQuery() for an LLM endpoint with the same signature.
...
function parseQuery(rawInput) {
  ...
  const countryNames = {
    ...
    SYR: ["syria"], AFG: ["afghanistan"], YEM: ["yemen"], NGA: ["nigeria"],
    KEN: ["kenya"], ETH: ["ethiopia"], COD: ["congo"], ZAF: ["south africa"],
    DEU: ["germany"], FRA: ["france"], GBR: ["united kingdom", "britain", "uk"],
    JPN: ["japan"], TWN: ["taiwan"], PHL: ["philippines"], VNM: ["vietnam"],
    MEX: ["mexico"], CAN: ["canada"], BRA: ["brazil"], COL: ["colombia"],
    VEN: ["venezuela"],
  };
  ...
  // ---- Unsatisfied / unplanned intent (handled BEFORE status to avoid 'unplanned' matching 'planned') ----
  ...
  // ---- Status (skip 'planned' match if 'unplanned' is in the query) ----
  const statusAliases = {
  ...
  Object.entries(statusAliases).forEach(([status, aliases]) => {
    // If 'unplanned' was found, suppress the 'Planned' status match
    if (status === "Planned" && isUnplanned) return;
    if (aliases.some((a) => new RegExp(`\\b${a}\\b`, "i").test(input))) {
  ...
  // ---- NSG members ----
  NSG_MEMBERS.forEach((m) => {
    const re = new RegExp(`\\b${m.toLowerCase()}\\b`, "i");
    if (re.test(input)) {
  ...
  // ---- Unsatisfied / unplanned: append filter chips and SQL clauses (booleans set above) ----
  if (isUnsatisfied) {
...
// ============================================================================
// ASKBAR COMPONENT — natural-language query input with SQL preview
// ============================================================================
...
const AskBar = ({ onApply, currentFilterCount }) => {
...
// ============================================================================
// GEOMAP COMPONENT — bubble map of product line volume by country
// ============================================================================
const GeoMap = ({ productLines }) => {
  // Aggregate by country GENC3
  const byCountry = useMemo(() => {
    const agg = {};
    productLines.forEach((p) => {
      if (!p.country) return;
      if (!agg[p.country]) {
        agg[p.country] = { genc3: p.country, total: 0, complete: 0, deviation: 0, aor: COUNTRY_TO_AOR[p.country] };
      }
      agg[p.country].total++;
      if (p.cur_prod_stat === "Complete") agg[p.country].complete++;
      if (p.deviation === 1) agg[p.country].deviation++;
    });
    return agg;
  }, [productLines]);

  // Map width/height — sized for the panel
  const W = 720;
  const H = 360;

  // Projection: Natural Earth (good for world overview, less distortion than Mercator)
  const projection = useMemo(
    () => geoNaturalEarth1().fitSize([W, H], WORLD_GEO),
    []
  );
  const pathGen = useMemo(() => geoPath(projection), [projection]);
  const graticule = useMemo(() => geoGraticule10(), []);

  // Centroid lookup for placing bubbles
  const countryPoints = useMemo(() => {
    return WORLD_GEO.features
      .map((f) => {
        const genc3 = ISO_NUM_TO_GENC3[String(f.id).padStart(3, "0")];
        if (!genc3 || !byCountry[genc3]) return null;
        const centroid = pathGen.centroid(f);
        if (!centroid || isNaN(centroid[0])) return null;
        return { ...byCountry[genc3], cx: centroid[0], cy: centroid[1], name: f.properties.name };
      })
      .filter(Boolean);
  }, [byCountry, pathGen]);

  // Bubble radius scale (sqrt to make area proportional to count)
  const maxTotal = Math.max(...countryPoints.map((p) => p.total), 1);
  const rScale = (n) => Math.max(3, Math.sqrt((n / maxTotal)) * 22);
  ...
  return (
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
        ...
        <rect x="0" y="0" width={W} height={H} fill={C.panelHi} />

        {/* Graticule (lat/long grid) */}
        <path d={pathGen(graticule)} fill="none" stroke={C.rule} strokeWidth="0.4" opacity="0.6" />

        {/* Countries */}
        <g>
          {WORLD_GEO.features.map((f, i) => {
            const genc3 = ISO_NUM_TO_GENC3[String(f.id).padStart(3, "0")];
            const data = genc3 ? byCountry[genc3] : null;
            const hasData = !!data;
            return (
              <path
                key={i}
                d={pathGen(f) || ""}
                fill={hasData ? "#ffffff" : "#fdfdfa"}
                stroke={C.rule}
                strokeWidth="0.5"
                style={{ cursor: hasData ? "pointer" : "default" }}
                onMouseEnter={() => hasData && setHover({ ...data, name: f.properties.name })}
        ...
        <g>
          {countryPoints
            .sort((a, b) => b.total - a.total) // largest first, smaller drawn on top
            .map((p, i) => {
              const aorColor = AOR_MAP_COLORS[p.aor] || C.steel;
              const devRatio = p.deviation / p.total;
              return (
                <g key={i} style={{ pointerEvents: "none" }}>
                  <circle
                    ...
                    strokeWidth="1.2"
                  />
                  {/* Inner ring shows deviation share */}
                  {devRatio > 0 && (
                    <circle
                      cx={p.cx}
                      cy={p.cy}
                      r={rScale(p.total) * Math.sqrt(devRatio)}
                      fill={C.rust}
                      fillOpacity={0.65}
                    />
                  )}
      ...
      {hover && (
        <div
          className="absolute top-2 right-2 px-3 py-2 pointer-events-none"
          style={{
            background: C.panel,
            border: `1px solid ${C.ruleHi}`,
            boxShadow: "0 4px 12px rgba(13,31,71,0.15)",
            minWidth: 180,
          }}
        >
          <div className="text-[13px] font-mono uppercase tracking-wider font-bold" style={{ color: C.text }}>
            {hover.name} <span className="font-medium" style={{ color: C.textFaint }}>· {hover.genc3}</span>
          </div>
          <div className="text-[12px] font-mono mt-1.5 font-semibold" style={{ color: C.textDim }}>
            AOR <span style={{ color: AOR_MAP_COLORS[hover.aor] || C.steel, fontWeight: 700 }}>{hover.aor}</span>
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-[13px] font-mono" style={{ color: C.text }}>
            <span style={{ color: C.textDim }}>Total:</span>
            <span className="tabular-nums font-bold">{hover.total}</span>
            <span style={{ color: C.textDim }}>Complete:</span>
            <span className="tabular-nums font-semibold" style={{ color: C.emerald }}>{hover.complete}</span>
            <span style={{ color: C.textDim }}>Unplanned:</span>
            <span className="tabular-nums font-semibold" style={{ color: C.rust }}>{hover.deviation}</span>
          </div>
        </div>
      )}
      ...
      {/* Legend */}
      <div
        className="flex items-center gap-4 mt-3 pt-3 px-1"
        style={{ borderTop: `1px solid ${C.rule}` }}
      >
        <div className="text-[11px] font-mono uppercase tracking-[0.16em] font-semibold" style={{ color: C.textDim }}>
          AOR
        </div>
        <div className="flex flex-wrap gap-x-3 gap-y-1">
          {Object.entries(AOR_MAP_COLORS).map(([aor, color]) => (
            <div key={aor} className="flex items-center gap-1.5 text-[12px] font-mono uppercase tracking-wider font-semibold" style={{ color: C.text }}>
              <span
                style={{
                  width: 10, height: 10, borderRadius: "50%",
                  background: color, opacity: 0.45, border: `1.2px solid ${color}`,
                }}
              />
              {aor}
            </div>
        ...
        <div className="ml-auto flex items-center gap-3 text-[12px] font-mono uppercase tracking-wider font-semibold" style={{ color: C.textDim }}>
          <span className="flex items-center gap-1.5">
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: C.rust, opacity: 0.7 }} />
            Unplanned share
          </span>
          <span className="flex items-center gap-1.5" style={{ color: C.textFaint }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 2 }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", border: `1.2px solid ${C.steel}`, background: "transparent" }} />
              <span style={{ width: 10, height: 10, borderRadius: "50%", border: `1.2px solid ${C.steel}`, background: "transparent" }} />
              <span style={{ width: 14, height: 14, borderRadius: "50%", border: `1.2px solid ${C.steel}`, background: "transparent" }} />
            </span>
            Volume
          </span>
        </div>
...
// ============================================================================
// GEOQUERY BRAND ELEMENTS
// ============================================================================

// Circuit-board globe logo — recreates the GeoQuery brand mark in SVG
const GeoQueryLogo = ({ size = 36 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
    ...
    {/* Globe outline */}
    <circle cx="50" cy="50" r="46" fill="none" stroke={C.limeDeep} strokeWidth="1.5" />
    {/* Latitude lines */}
    <g stroke={C.limeDeep} strokeWidth="0.8" fill="none" clipPath="url(#globeClip)" opacity="0.85">
      ...
      <ellipse cx="50" cy="50" rx="28" ry="46" />
      <ellipse cx="50" cy="50" rx="42" ry="46" />
    </g>
    {/* Circuit traces inside the globe */}
    <g stroke={C.limeDeep} strokeWidth="1.2" fill="none" clipPath="url(#globeClip)" strokeLinecap="round" strokeLinejoin="round">
      <path d="M 15 35 L 25 35 L 30 30 L 42 30 L 45 33" />
      <path d="M 60 25 L 72 25 L 76 29 L 82 29" />
      <path d="M 18 55 L 28 55 L 32 51 L 42 51 L 46 55 L 54 55" />
      <path d="M 58 65 L 68 65 L 72 69 L 82 69" />
      <path d="M 20 75 L 32 75 L 36 71 L 48 71" />
      <path d="M 55 80 L 70 80 L 75 75" />
      <path d="M 30 18 L 30 28 M 70 18 L 70 22" />
    </g>
    {/* Circuit nodes (dots) */}
    <g fill={C.limeDeep} clipPath="url(#globeClip)">
      <circle cx="25" cy="35" r="1.6" />
      <circle cx="42" cy="30" r="1.6" />
      <circle cx="60" cy="25" r="1.6" />
      <circle cx="82" cy="29" r="1.6" />
      <circle cx="28" cy="55" r="1.6" />
      <circle cx="46" cy="55" r="1.6" />
      <circle cx="68" cy="65" r="1.6" />
      <circle cx="82" cy="69" r="1.6" />
      <circle cx="32" cy="75" r="1.6" />
...
// Decorative circuit-board background traces — fixed-position, subtle, lime
const CircuitBackground = () => (
  ...
  >
    <g stroke={C.ruleHi} strokeWidth="1" fill="none" strokeLinecap="round" strokeLinejoin="round">
      {/* Top-right — single subtle trace */}
      <path d="M 1600 60 L 1500 60 L 1480 80 L 1380 80" />
      {/* Bottom-left — single subtle trace */}
      <path d="M 0 940 L 120 940 L 140 920 L 240 920" />
    </g>
  </svg>
);

// ============================================================================
export default function RMTDashboard() {
  ...
  const [filters, setFilters] = useState({
    domain: [], aor: [], country: [], fy: [], nsg: [], priority: [], status: [],
  });
  ...
  const insights = useMemo(() => (filtered && kpis ? generateInsights(filtered, kpis) : []), [filtered, kpis]);

  const sparkData = useMemo(
    () => (filtered ? filtered.counts.slice(-12).map((c) => ({ v: c.count_complete })) : []),
    [filtered]
  );
  const sparkInWork = useMemo(
    () => (filtered ? filtered.counts.slice(-12).map((c) => ({ v: c.count_in_work })) : []),
    [filtered]
  );

  const totalActiveFilters = Object.values(filters).reduce((a, b) => a + b.length, 0);

  const setFilter = (key, value) => setFilters((f) => ({ ...f, [key]: value }));
  const clearAll = () =>
    setFilters({ domain: [], aor: [], country: [], fy: [], nsg: [], priority: [], status: [] });
  ...
  return (
    ...
    >
      {/* Embedded font + tiny utility CSS */}
      <style>{`
        ...
        .font-serif { font-family: 'Fraunces', Georgia, serif; font-variation-settings: "opsz" 60; }
        .font-mono, table, th, td { font-family: 'JetBrains Mono', ui-monospace, monospace; }
        body { background: ${C.ink}; }
        ::selection { background: ${C.amber}; color: ${C.invert}; }
      ...
      <div className="desktop-warning hidden lg:!hidden items-center justify-center min-h-screen p-8 text-center">
        <div>
          <Activity size={32} style={{ color: C.amber }} className="mx-auto mb-4" />
          <div className="font-serif text-2xl mb-2">Best Viewed on Desktop</div>
          <div className="text-sm" style={{ color: C.textDim }}>
            The RMT Dashboard requires a 1024px+ viewport for proper rendering of charts and matrices.
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:block relative">
        ...
        {/* HEADER */}
        <header style={{ borderBottom: `2px solid ${C.ruleHi}`, background: C.panel, position: "relative", zIndex: 10 }}>
          <div className="px-6 py-4 flex items-center gap-6">
            <div className="flex items-center gap-3">
              <GeoQueryLogo size={44} />
              <div>
                <div className="font-serif text-[26px] font-bold" style={{ color: C.amber, letterSpacing: "0.02em", lineHeight: 1 }}>
                  GEOQUERY
                </div>
                <div className="text-[11px] font-mono uppercase tracking-[0.2em] mt-1" style={{ color: C.textDim }}>
                  RMT Dashboard · NGA
                </div>
              </div>
            </div>

            <div className="h-9 w-px" style={{ background: C.rule }} />

            <div className="flex items-center gap-2">
              <span className="text-[11px] font-mono uppercase tracking-[0.16em] font-semibold" style={{ color: C.textDim }}>
                Office
              </span>
              <select
                value={office}
                onChange={(e) => setOffice(e.target.value)}
                className="bg-transparent text-[13px] font-mono px-2.5 py-1.5"
                style={{ border: `1px solid ${C.rule}`, color: C.text }}
              >
                {OFFICES.map((o) => (
                  <option key={o.code} value={o.code} style={{ background: C.panel }}>
                    {o.code} — {o.label}
                  </option>
                ))}
              </select>
            ...
            <div className="ml-auto flex items-center gap-4">
              <div className="text-right">
                ...
                <div className="text-[12px] font-mono tabular-nums mt-0.5" style={{ color: C.text }}>
                  {lastRefresh.toLocaleTimeString()} · {lastRefresh.toLocaleDateString()}
                </div>
              </div>
              <button
                onClick={() => setOffice((o) => o)}
                onMouseDown={() => {
                  setLoading(true);
                  loadAllServices(office).then((d) => {
                    setData(d);
                    setLoading(false);
                    setLastRefresh(new Date());
                  });
                }}
                className="flex items-center gap-2 text-[12px] font-mono uppercase tracking-wider font-semibold px-3 py-2"
              ...
              <div
                className="text-[11px] font-mono uppercase tracking-[0.16em] font-semibold px-2.5 py-1.5"
                style={{
                  border: `1.5px solid ${USE_MOCK_DATA ? C.rust : C.emerald}`,
                  color: USE_MOCK_DATA ? C.rust : C.emerald,
                }}
              >
                {USE_MOCK_DATA ? "MOCK MODE" : "LIVE"}
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="flex" style={{ minHeight: "calc(100vh - 57px)" }}>
          {/* SIDEBAR */}
          <aside
            className="flex-shrink-0 px-5 py-5 overflow-y-auto"
            style={{
              width: filterCollapsed ? 56 : 250,
          ...
          >
            <button
              ...
              className="flex items-center gap-2.5 w-full mb-5 text-[13px] tracking-[0.16em] uppercase font-mono font-bold"
              style={{ color: C.amber }}
            >
              <Filter size={15} />
              {!filterCollapsed && (
            ...
            {!filterCollapsed && (
              <>
                {totalActiveFilters > 0 && (
                  <button
                    onClick={clearAll}
                    className="text-[11px] font-mono uppercase tracking-wider mb-4 hover:opacity-100 font-semibold"
                    style={{ color: C.amber }}
                  >
                    × Clear all filters
                  </button>
                ...
                <FilterGroup
                  label="AOR"
                  options={AORS}
                  selected={filters.aor}
                  onChange={(v) => setFilter("aor", v)}
                />
                <FilterGroup
                  label="Country"
                  options={[...new Set(Object.values(COUNTRIES_BY_AOR).flat())].sort()}
                ...
                <FilterGroup
                  label="NSG Member"
                  options={NSG_MEMBERS}
                  selected={filters.nsg}
                  onChange={(v) => setFilter("nsg", v)}
                />
                <FilterGroup
                  label="Priority Tier"
          ...
          <main className="flex-1 min-w-0">
            ...
            {activeChips.length > 0 && (
              <div
                ...
                style={{ borderBottom: `1px solid ${C.rule}`, background: C.panel }}
              >
                <span className="text-[12px] font-mono uppercase tracking-[0.18em]" style={{ color: C.textFaint }}>
                  Active Filters:
                </span>
                {activeChips.map((c) => (
                  <Tag
                    ...
                    onRemove={() =>
                      setFilter(
                        c.keyName,
                        filters[c.keyName].filter((x) => x !== c.value)
                      )
                    }
                  >
                    {c.label}
            ...
            <div className="p-6 space-y-4" style={{ background: "transparent" }}>
              {/* AI: Natural Language Query Bar */}
              <AskBar
                onApply={(parsedFilters) => {
                  setFilters(parsedFilters);
                }}
                currentFilterCount={totalActiveFilters}
              />

              <div className="space-y-px" style={{ background: C.rule }}>
              {/* KPI strip */}
              {kpis && (
                <div className="grid grid-cols-6 gap-px" style={{ background: C.rule }}>
                  <KPITile
                    label="Total Product Lines"
                    value={kpis.total.toLocaleString()}
                    sub={`Across ${OFFICES.find((o) => o.code === office)?.code}`}
                    icon={Layers}
                  />
                  <KPITile
                    label="% Complete (FY25)"
                    value={kpis.pctComplete.toFixed(1)}
                    suffix="%"
                    sub="Of total volume"
                    spark={sparkData}
                  ...
                  <KPITile
                    label="In-Work Count"
                    value={kpis.inWork.toLocaleString()}
                    sub="cur_prod_stat = 3"
                    spark={sparkInWork}
                    sparkColor={C.cream}
                    icon={Clock}
                  ...
                  <KPITile
                    ...
                    value={kpis.unplanned.toLocaleString()}
                    sub={`${kpis.pctUnplanned.toFixed(1)}% of total`}
                    icon={AlertTriangle}
                    tone="warn"
                  />
                  <KPITile
                    ...
                    sub="Active in current scope"
                    icon={FileText}
                  />
                  <KPITile
                    label="High-Pri Unsatisfied"
                    value={kpis.highPriUnsat.toLocaleString()}
                    sub={`${kpis.pctHighPriUnsat.toFixed(1)}% of all reqs`}
                    icon={ArrowUpRight}
                    tone="alert"
                  />
                </div>
              ...
              {/* Tab bar */}
              <div className="flex" style={{ background: C.panel, borderBottom: `1px solid ${C.rule}` }}>
                {[
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-2.5 px-6 py-4 text-[13px] font-mono uppercase tracking-[0.14em] font-semibold relative"
                      style={{
                    ...
                    >
                      ...
                      {active && (
                        <span
                          ...
                          style={{ background: C.amber, height: "2px" }}
                        />
                      )}
                    </button>
                  );
                })}
                <div className="ml-auto px-5 py-4 text-[11px] font-mono uppercase tracking-wider font-semibold flex items-center gap-2" style={{ color: C.textDim }}>
                  <span style={{ width: 7, height: 7, borderRadius: "50%", background: C.emerald }} />
                  Endpoint OK
                </div>
              </div>

              {/* Tab content + insights rail */}
              <div className="grid grid-cols-12 gap-px" style={{ background: C.rule }}>
                <div className="col-span-12 xl:col-span-9">
                  {loading && (
                    <div className="p-16 text-center" style={{ background: C.panel }}>
                      <RefreshCw size={24} className="animate-spin mx-auto mb-4" style={{ color: C.amber }} />
                      <div className="text-[14px] font-mono uppercase tracking-[0.16em] font-semibold" style={{ color: C.text }}>
                        Querying {office} services…
                      </div>
                    </div>
                  )}
                  {error && (
                    <div className="p-10" style={{ background: C.panel }}>
                      <div className="font-serif text-2xl mb-3 font-semibold" style={{ color: C.rust }}>
                        Service Error
                      </div>
                      <div className="text-[13px] font-mono" style={{ color: C.textDim }}>
                        {error}
                      </div>
                    </div>
                  )}
                  {filtered && !loading && !error && (
                    <>
                      {activeTab === "production" && <ProductionTab filtered={filtered} />}
                      {activeTab === "requirements" && <RequirementsTab filtered={filtered} />}
                      {activeTab === "missions" && <MissionsTab filtered={filtered} />}
                    </>
                  )}
                </div>

                {/* Insights rail */}
                <div className="col-span-12 xl:col-span-3" style={{ background: C.panel }}>
                  <div className="px-5 pt-4 pb-3 flex items-center gap-2">
                    <div
                      className="text-[14px] tracking-[0.14em] uppercase font-mono font-bold"
                      style={{ color: C.amber }}
                    >
                      ◆ Key Insights
                    </div>
                  ...
                  <div className="px-5 pb-5 space-y-4">
                    {insights.map((insight, i) => {
                      const tones = {
                        warn: { color: C.rust, icon: AlertTriangle },
                        alert: { color: C.rose, icon: AlertTriangle },
                        good: { color: C.emerald, icon: TrendingUp },
                        neutral: { color: C.steel, icon: Minus },
                      };
                      ...
                      return (
                        <div key={i} className="flex gap-3">
                          <div className="flex-shrink-0 mt-1">
                            <Icon size={15} style={{ color: t.color }} strokeWidth={2.2} />
                          </div>
                          <div
                    ...
                    {insights.length === 0 && (
                      <div className="text-[13px] font-mono" style={{ color: C.textFaint }}>
                        No insights available — adjust filters.
                      </div>
                    )}
                    <div className="pt-4 mt-2" style={{ borderTop: `1px dashed ${C.rule}` }}>
                      <div className="text-[12px] font-mono uppercase tracking-[0.14em] font-bold mb-2" style={{ color: C.textDim }}>
                        Methodology
                      </div>
                      <div className="text-[12px] leading-relaxed" style={{ color: C.textDim, fontStyle: "italic" }}>
                        Insights are recomputed live from the four dash_* services using the active filter set.
                        Derived priority follows the RMT rule: cust_pri ≤ 2 AND risk_factor ≤ 2.
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Footer */}
              <div
                className="px-4 py-2.5 text-[11px] font-mono uppercase tracking-[0.16em] font-semibold flex items-center justify-between"
                style={{ background: C.panel, color: C.textFaint }}
              >
                <div>
                  Source: <span style={{ color: C.textDim }}>{REST_BASE}/{office}/MapServer/[0–3]</span>
                </div>
                <div className="flex items-center gap-4">
                  <span>dash_ProductLines · dash_Requirements · dash_Missions · dash_Counts</span>
                  <span>FOUO // RMT v2.4</span>
                </div>
              </div>
              </div>
            </div>
